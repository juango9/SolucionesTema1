...
```{r}
#b
#La previsualizacion del conjunto de datos mammals con la libreria MASS
library(MASS)
dim(mammals)
str(mammals)
head(mammals)
...
```
```{r}
#La previsualizacion del conjunto de datos Animals2 con la libreria robustbase
library(robustbase)
dim(Animals2)
str(Animals2)
head(Animals2)
...
```{r}
#Estos son los animales de mammals que no estan en Animals2
diferentAnimals1<-(setdiff(rownames(mammals),rownames(Animals2)))
diferentAnimals1
#Aqui hay 4 diferentes
#Estos son los animales de Animals2 que no estan en mammals
diferentAnimals2<-(setdiff(rownames(Animals2),rownames(mammals)))
diferentAnimals2
#Aqui hay 7 diferentes
#Hay un total de 11 animales diferentes
...
library(MASS)
library(robustbase)
CerebroMammals<-qqPlot(log(mammals$brain))
CerebroMammals
```

```{r}
CerebroAnimals<-qqPlot(log(Animals2$brain))
CerebroAnimals
```
Si las dos graficas presentan el mismo comportamiento y diremos que ninguna de ellas es similar a una distribucion Gaussiana puesto que esta es en forma de campana y las graficas representadas no.
...
#Su estrutura es una lista formada por 3 elementos
elem<-head(libReturn$results, n=5)
elem
#En elem se muestran los 5 primeros elementos de la matriz 
#c
dim(libReturn$results)
#sabiendo la dimension, sabemos que tiene 188 columanas y 3 filas como la primera col se corresponde con el nombre de la s librias
#diremos que hay 188 librerias intaladas
...
```{r}
#b
x<-anyNA(cabbages, recursive = FALSE)
x
```
```{r}
#c
